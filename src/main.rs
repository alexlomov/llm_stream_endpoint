...
use llm_stream::llm::llm::{generate, LlmPackage, LLM};

#[cfg(feature = "llama")]
use llm_stream::llm::quantized_llm::{generate, QuantizedLLM, QuantizedLlmPackage};
...
async fn main() -> anyhow::Result<()> {
    ...
    let context =
        get_prompt_context(config_file_name, args_init.context_type.clone()).to_lowercase();
    ...
    let routes_generation = warp::path("token_stream")
        ...
        .map(move |prompt: Prompt| {
            ...
            let _ = dedicated_runtime_clone
                .lock()
                .unwrap()
                .as_ref()
                .unwrap()
                .spawn(async move {
                    process_generation(llm_package_clone, prompt.query, tx, context.to_lowercase())
                        .await;
                });

            let event_stream = rx_stream.map(move |token| Ok(Bytes::from(token)));
    ...
    warp::serve(routes_generation.or(routes_index))
        .run(([127, 0, 0, 1], 3030))
        .await;
